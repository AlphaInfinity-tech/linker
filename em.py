...
HTML_PATH = os.path.join(BASE_DIR, 'em.html')
...
def init_db():
    with db_lock:
        ...
        # Default campaign if none exists
...
class SafeServerHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        try:
            ...
            # Serve Client Tracker HTML
            if path == '/' or path == '/index.html':
                if not os.path.exists(HTML_PATH):
                    self._send_response(500)
                    self.wfile.write(json.dumps({'error': 'em.html not found'}).encode('utf-8'))
                    return
            ...
            if path == '/admin' or path == '/admin.html':
                if not os.path.exists(ADMIN_PATH):
                    self._send_response(500)
                    self.wfile.write(json.dumps({'error': 'admin.html not found'}).encode('utf-8'))
                    return
            ...
            # API: List Sessions with aggregated metrics
            if path == '/api/sessions':
                with db_lock:
                    ...
                    # Calculate total distance for each session
            ...
            # API: Single Session details with trail
            ...
            if path == '/api/export':
                ...
                if fmt == 'geojson':
                    features = []
                    # Group locations by token for LineStrings
                    grouped_locs = {}
                    for l in loc_rows:
                        grouped_locs.setdefault(l['token'], []).append(l)

                    # Point features for all fixes
                    for l in loc_rows:
                        features.append({
                            'type': 'Feature',
                            'geometry': {
                    ...
                    # LineString trails for each session
            ...
            # Legacy Consents endpoint (for backward compatibility)
            if path == '/api/consents':
                with db_lock:
                    conn = get_db_connection()
                    c = conn.cursor()
                    c.execute('SELECT token, created_at as timestamp, user_agent, origin FROM sessions ORDER BY created_at DESC')
                    rows = [dict(r) for r in c.fetchall()]
                    conn.close()
                self._send_response()
                self.wfile.write(json.dumps(rows).encode('utf-8'))
    ...
    def do_POST(self):
        try:
            ...
            # Register Consent & Fingerprint: POST /consent or POST /api/consent
            if path in ('/consent', '/api/consent'):
                ...
                print(f"[CONSENT REGISTERED] Token: {token} | IP: {client_ip} | Campaign: {campaign_code}")
                self._send_response()
                self.wfile.write(json.dumps({'status': 'consent_recorded', 'token': token}).encode('utf-8'))
                return

            # Submit Location Fix: POST /submit-location or POST /api/submit-location
            if path in ('/submit-location', '/api/submit-location'):
                ...
                with db_lock:
                    ...
                    # Verify session exists, if not auto-register default
                ...
                print(f"[LOCATION FIX] Token: {token} | Lat: {lat}, Lon: {lon} | Acc: {data.get('accuracy')}m | Spd: {data.get('speed')}")
                self._send_response()
...
def run_server(host='', port=8000):
    ...
    httpd = HTTPServer(server_address, SafeServerHandler)
    print("\n=======================================================")
    print("  [GEOTRACE] Advanced Telemetry & Tracking Server")
    print("=======================================================")
    print(f"  * Client Tracker  : http://localhost:{port}/")
    print(f"  * Admin Dashboard : http://localhost:{port}/admin")
    print(f"  * Persistence     : SQLite ({DB_PATH})")
    print("  * Real-time sync  : watchPosition & live breadcrumbs")
    print("=======================================================\n")
    try:
        httpd.serve_forever()
